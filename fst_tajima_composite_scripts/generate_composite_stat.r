...
write.table(top_windows, file.path(outdir, "composite_score.additive.0.1perc.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
...
top_windows <- df_clean[df_clean$composite_score >= cutoff, ]
df_clean$highest_composite <- df_clean$composite_score >= cutoff
write.table(top_windows, file.path(outdir, "composite_score.additive.1perc.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)


# write full table to tsv
write.table(df_clean, file.path(outdir, "composite_score.additive.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)

df_clean$highlight_group <- "None"
df_clean$highlight_group[df_clean$highest_composite] <- "CS top 0.1%"

# Plot and save to PDF
pdf(file.path(outdir, "additive_composite_score.scaled.pdf"), width = 8, height = 6)
...
# Plot and save to PDF
pdf(file.path(outdir, "additive_composite_score.pdf"), width = 8, height = 6)
